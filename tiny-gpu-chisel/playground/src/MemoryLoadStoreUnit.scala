...
class MemLoadStoreUnit extends Module {
    val io = IO(new Bundle {
        ...
        val reg_in = new Bundle {
            val rs = Input(UInt(8.W))  // Base address
            val rt = Input(UInt(8.W))  // Data to write (for store)
        }

        val mem_read_data = Input(UInt(8.W))
        val mem_read_address_sender = new DecoupledIO(UInt(8.W))  // DecoupledIO(Ready-Valid握手协议)
    ...
    when(io.enable) {
        ...
        when(io.mem_rw_enable.read_enable) {
            switch(io.lsu_state) {
                ...
                is(LSUState.DONE) {
                    when(io.core_state === CoreState.DONE) {
...
object MainLSU extends App {
    println(getVerilogString(new MemLoadStoreUnit))
}
