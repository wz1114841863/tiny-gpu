...
class Scheduler(ThreadsPerBlock: Int = 4) extends Module {
    val io = IO(new Bundle{
        ...
        val lsu_state = Input(LSUState())

        // Current & Next PC
        val current_pc = Input(UInt(8.W))
        val next_pc = Output(UInt(8.W))
    ...
    when(!reset.asBool) {
        switch(core_state) {
            ...
            is(CoreState.WAIT) {
                // wait for all LSUs to finish their requests
                val any_lsu_waiting = io.lsu_state.exists(lsu_state => lsu_state === LSUState.WAITING || lsu_state === LSUState.REQUESTING)
